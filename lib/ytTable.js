...
function getCanFocusChildNodes(dom) {
    return dom && Array.from(dom.childNodes).filter(function (node) {
        return node.className.indexOf(ytTablePerfix$6 + '-can-focus-cell') > -1;
    }) || [];
}
...
var getNextFocus = function getNextFocus(dom, selector) {
    ...
    var pIndex = getDomIndex(pNode, '.' + ytTablePerfix$6 + '-row');
    var nextNode = null;
    if (selfIndex < cfaLen - 1) {
        nextNode = canFoucesArr[selfIndex + 1];
    } else {
        var nextRow = document.querySelector('.' + ytTablePerfix$6 + '-row:nth-child(' + (pIndex + 2) + ')');
...
function accArrWidth() {
    ...
    var getObjNum = function getObjNum(obj, keyCall) {
        var item = obj;
        if (keyCall) {
            item = keyCall(obj);
        }
	...
	TableRow.prototype.render = function render() {
		...
		return React.createElement(
			'div',
			{
				...
				}, _ref['width'] = accArrWidth(columns, function (record) {
					return record.width;
				}), _ref),
    ...
    function TableHeaderCell() {
        ...
        return _ret = (_temp = (_this = possibleConstructorReturn(this, _React$Component.call.apply(_React$Component, [this].concat(args))), _this), _this.isMouseDown = false, _this.handleMouseDown = function (e) {
            ...
            if (_this.isMouseDown && dom) {
                var posObj = dom.getBoundingClientRect();
                var width = e.pageX - posObj.left + 'px';
...
var defaultCellWidth$1 = config.defaultCellWidth;
			...
			if (!columsObj[column.fatherTitle.title]) {
				try {
					...
					var fatherWidth = subCols.reduce(function (pre, cur) {
						return pre + (cur.width || defaultCellWidth$1);
					}, 0);
					useArr.push({
...
function TableHeader(props) {
	...
	var getHeaderCellCls = function getHeaderCellCls() {
		...
		var draggable = arguments[1];

		return _ref = {}, _ref[ytTablePerfix$7 + '-thead-cell-mul'] = isMul, _ref[ytTablePerfix$7 + '-thead-cell'] = !isMul, _ref[ytTablePerfix$7 + '-up-down-center'] = !isMul, _ref[ytTablePerfix$7 + '-thead-cell-draggable'] = !isMul && draggable, _ref;
	};
	var subCellCls = (_subCellCls = {}, _subCellCls[ytTablePerfix$7 + '-thead-cell-mul-bottom-item'] = true, _subCellCls[ytTablePerfix$7 + '-up-down-center'] = true, _subCellCls[ytTablePerfix$7 + '-thead-cell-draggable'] = true, _subCellCls);
	...
	return React.createElement(
		'div',
		{
			...
			className: ytTablePerfix$7 + '-thead'
		},
		useArr.map(function (item) {
			...
			if (item.isFather) {
				...
				return React.createElement(
					...
					React.createElement(
						'div',
						{
							className: ytTablePerfix$7 + '-thead-cell-mul-top ' + ytTablePerfix$7 + '-up-down-center'
						},
						title
					),
					React.createElement(
						'div',
						{
							className: ytTablePerfix$7 + '-thead-cell-mul-bottom'
						},
						subCols.map(function (ele) {
							return React.createElement(
								...
								_extends({}, props, {
									...
									columnKey: ele.key,
									style: {
										flex: getFlexWidth(ele.width || defaultCellWidth$1)
									},
			...
			return React.createElement(
				...
				_extends({}, props, {
					...
					columnKey: key,
					style: {
						flex: getFlexWidth(width || defaultCellWidth$1)
					},
...
var Table = function (_React$Component) {
	...
	Table.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
		var compareArr = ['columns', 'dataSource'];
