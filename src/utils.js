...
export function genInitResize({ columns }) {
    return columns.map(genItemResize);
}

export function genInitConfig(columns) {
 ...
 * @param {*} getKey
 */
export function checkTwoArrIndexKeyEqual(arr1, arr2, getKey) {
...
function genNewConfigFromOld(newColumn, oldColumn, oldResized) {
    ...
    const resized = flatColumns.map(item => {
        ...
        if (oldItem) {
            const oldResize = oldResized.find(it => it.key === item.key);
            if (oldResize.value) {
                return oldResize;
            }
        }
 ...
 * }
 */
export function genNewConfigByOld({ newColumn, oldColumn, oldResized }) {
    if (newColumn !== oldColumn) {
        ...
        if (newColumn.length !== oldColumn.length || flatColumns.length !== oldFlatColumns.length) {
            ...
            if (checkTwoArrIndexKeyEqual(newColumn, oldColumn, item => item.key)) {
                return {
...
function genResizeByFlatCol({ currentlyResizing, columns, newWidth, resized }) {
 ...
 * @param {*} param0
 */
function genResizeByNestCol({ currentlyResizing, newWidth, resized, columns, column }) {
    ...
    const oldCellGroupStyl = getCellGroupStyl({ siblingTitleArr: children, resized });
 ...
 */
function genResizeWithSiblingCell({ currentlyResizing, newWidth, resized, pColumn }) {
...
export function genResize({ currentlyResizing, newWidth, resized, pColumn, column = {} }) {
    const children = column.children;
    if (children && Array.isArray(children)) {
        return genResizeByNestCol({ currentlyResizing, newWidth, resized, column });
...
export function getNumWidth(width) {
...
export function getRowMinWidth({ resized, rowSelection }) {
...
export function getCellWidthStyl({ curResizedObj, styles }) {
...
export function getCellGroupStyl({ siblingTitleArr, resized }) {
    const siblingResize = resized.filter(item => siblingTitleArr.find(it => it.key === item.key));
 ...
 * @param {*} value 整数。。。
 */
export function genWidthVal(value) {
    return Math.round(value);
}

export function getKey(data, rowkey) {
...
export function getCanCheckAllNestKeys({ dataSource, childrenName, rowKey, getCheckboxProps }) {
    const filterCb = it => (getCheckboxProps ? !(getCheckboxProps(it) || {}).disabled : true);
    let arr = [];
    const loop = data => {
        data.forEach(item => {
...
function checkColumnsHasWidth(column) {
    return column.width || (column.styles && (column.styles.width || column.styles.maxWidth))
}
...
export function adjustResized(rowDom, config = {}) {
    ...
    const childList = [...rowDom.childNodes].filter(it => it.className.indexOf('yc-tbody-row-select-cell') === -1)
    return resized.map((it, index) => {
        if (!checkColumnsHasWidth(flatColumns[index]) && childList[index].clientWidth) {
            return {
                ...it,
 ...
 * @param {*} ele
 */
export function isLastChild(arr, ele) {
...
export function accReduce(num1, num2) {
    const m = preAcc(num1, num2);
    return Math.round((num1 * m) - (num2 * m)) / m;
}
